...
public class CheatCheckerManager {
    ...
    public void startCheck(CommandSender initiator, Player player, @Nullable String reason, int checkTime) {
        ...
        initiator.sendMessage(TextUtils.setColors(getFileAccessor().getLang().getString("commands.cheatcheck.success").replace("%1$f", player.getName())));
        player.sendMessage(TextUtils.setColors(getFileAccessor().getLang().getString("commands.cheatcheck.chat-message-on-start").replace("%1$f", initiatorName).replace("%2$f", reason).replace("%3$f", String.valueOf(checkTime))));
        if(getConfigSettings().isSendTitleOnCheatCheck()) {
            CoreAdapter.getAdapter().sendTitle(player, TextUtils.setColors(getFileAccessor().getLang().getString("commands.cheatcheck.header-title-on-start")), TextUtils.setColors(String.join("\n", getFileAccessor().getLang().getStringList("commands.cheatcheck.footer-title-on-start"))));
        }
        this.countdown.put(player, checkTime);
        TemporaryCache.setCheckingPlayersNames(player.getName());
        BaseManager.getBaseManager().insertIntoHistory(getFileAccessor().getLang().getString("other.history-formats.cheatcheck").replace("%1$f", initiatorName).replace("%2$f", player.getName()).replace("%3$f", reason).replace("%4$f", getDate() + ", " + getTime()));
    ...
    public void stopCheck(CommandSender initiator, Player player) {
        if(isPlayerChecking(player)) {
            if(getCheckingCheatsPlayers().getInitiatorsAndHisPlayers().get(player) == initiator) {
                ...
                player.sendMessage(TextUtils.setColors(getFileAccessor().getLang().getString("commands.cheatcheck.chat-message-on-stop")));
                if(getConfigSettings().isSendTitleOnCheatCheck()) {
                    CoreAdapter.getAdapter().sendTitle(player, TextUtils.setColors(getFileAccessor().getLang().getString("commands.cheatcheck.header-title-on-stop")), TextUtils.setColors(String.join("\n", getFileAccessor().getLang().getStringList("commands.cheatcheck.footer-title-on-stop"))));
                }
                TemporaryCache.unsetCheckingPlayersNames(player.getName());
                initiator.sendMessage(TextUtils.setColors(getFileAccessor().getLang().getString("commands.cheatcheck.check-stopped").replace("%1$f", player.getName())));
            } else {
        ...
        TimerTask timerTask = new TimerTask() {
            ...
            public void run() {
                if(player != null && isPlayerChecking(player)) {
                    ...
                    CoreAdapter.getAdapter().sendActionBar(player, TextUtils.setColors(getFileAccessor().getLang().getString("commands.cheatcheck.actionbar-message").replace("%1$f", timeSettingsAccessor.getTimeManager().convertFromMillis(timeSettingsAccessor.getTimeManager().convertFromSecToMillis(tLeft)))));
                    if(tLeft <= 0) {
