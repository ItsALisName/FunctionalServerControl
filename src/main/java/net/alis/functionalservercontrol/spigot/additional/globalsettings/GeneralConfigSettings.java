...
public class GeneralConfigSettings {
    ...
    public void loadConfigSettings() {
        ...
        TaskManager.preformAsync(() -> {
            ...
            //Tasks
            if(isDupeIdModeEnabled()) {
                TaskManager.preformAsyncTimerTask(new DupeIpTask(), 0, getDupeIpTimerDelay() * 20L);
            }
            TaskManager.preformAsyncTimerTask(new CooldownsTask(), 0, 20L);
            TaskManager.preformAsyncTimerTask(new MuteGlobalTask(), 0, 20L);
            if(isPermissionsProtectionEnabled()) {
                TaskManager.preformAsyncTimerTask(new PermissionsControlTask(), 0, getPermissionsProtectionDelay() * 20L);
            }
            if(getChatSettings().isFunctionEnabled()) {
                TaskManager.preformAsyncTimerTask(new ChatTask(), 0, 20);
            }
