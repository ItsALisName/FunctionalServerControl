                ...
                if(args.length == 2) {
                    if(timeSettingsAccessor.getTimeChecker().checkInputTimeArgument(args[1])) {
                            ...
                            Bukkit.getScheduler().runTaskAsynchronously(this.plugin, () -> {
                                if(OtherUtils.isNotNullIp(args[0])) {
                                    if(OtherUtils.getPlayerByIP(args[0]) != null) {
                                        ...
                                        banManager.preformBan(player, BanType.TIMED_NOT_IP, getGlobalVariables().getDefaultReason(), sender, time, true, "tempban");
                                        return;
                                    } else {
                                        if(!sender.hasPermission("functionalservercontrol.temp-ban.offline")) {
                                            sender.sendMessage(setColors(getFileAccessor().getLang().getString("other.offline-no-perms")));
                                        ...
                                        banManager.preformBanByIp(args[0], BanType.TIMED_NOT_IP, getGlobalVariables().getDefaultReason(), sender, time, true, "tempban", false);
                                        return;
                                    }
                                } else {
                                    if(!sender.hasPermission("functionalservercontrol.temp-ban.offline")) {
                                    ...
                                    banManager.preformBanByIp(args[0], BanType.TIMED_NOT_IP, getGlobalVariables().getDefaultReason(), sender, time, true, "tempban", true);
                                    return;
                                }
                            });
                            return true;
                        ...
                        if(!OtherUtils.isNotNullPlayer(args[0])) {
                            ...
                            Bukkit.getScheduler().runTaskAsynchronously(this.plugin, () -> {
                                banManager.preformBan(args[0], BanType.TIMED_NOT_IP, getGlobalVariables().getDefaultReason(), sender, time, true, "tempban");
                            });
                            return true;
                        } else {
                            if(!player.isOnline()) {
                ...
                if(args.length >= 3) {
                    ...
                    if(args.length == 3 && args[0].equalsIgnoreCase("-s")) {
                        if(timeSettingsAccessor.getTimeChecker().checkInputTimeArgument(args[2])) {
                                ...
                                Bukkit.getScheduler().runTaskAsynchronously(this.plugin, () -> {
                                    if(OtherUtils.isNotNullIp(args[1])) {
                                        if(OtherUtils.getPlayerByIP(args[1]) != null) {
                                            ...
                                            banManager.preformBan(player, BanType.TIMED_NOT_IP, getGlobalVariables().getDefaultReason(), sender, time, false, "tempban");
                                            return;
                                        } else {
                                            if(!sender.hasPermission("functionalservercontrol.temp-ban.offline")) {
                                                sender.sendMessage(setColors(getFileAccessor().getLang().getString("other.offline-no-perms")));
                                            ...
                                            banManager.preformBanByIp(args[1], BanType.TIMED_NOT_IP, getGlobalVariables().getDefaultReason(), sender, time, false, "tempban", false);
                                            return;
                                        }
                                    } else {
                                        if(!sender.hasPermission("functionalservercontrol.temp-ban.offline")) {
                                        ...
                                        banManager.preformBanByIp(args[1], BanType.TIMED_NOT_IP, getGlobalVariables().getDefaultReason(), sender, time, false, "tempban", true);
                                        return;
                                    }
                                });
                                return true;
                            ...
                            if(!OtherUtils.isNotNullPlayer(args[1])) {
                                ...
                                Bukkit.getScheduler().runTaskAsynchronously(this.plugin, () -> {
                                    banManager.preformBan(args[1], BanType.TIMED_NOT_IP, getGlobalVariables().getDefaultReason(), sender, time, false, "tempban");
                                });
                                return true;
                            } else {
                                if(!player.isOnline()) {
                    ...
                    if(args.length > 3 && args[0].equalsIgnoreCase("-s")) {
                        ...
                        if (timeSettingsAccessor.getTimeChecker().checkInputTimeArgument(args[2])) {
                                ...
                                Bukkit.getScheduler().runTaskAsynchronously(this.plugin, () -> {
                                    if(OtherUtils.isNotNullIp(args[1])) {
                                        if(OtherUtils.getPlayerByIP(args[1]) != null) {
                                            ...
                                            banManager.preformBan(player, BanType.TIMED_NOT_IP, getReason(args, 3), sender, time, false, "tempban");
                                            return;
                                        } else {
                                            if (!sender.hasPermission("functionalservercontrol.temp-ban.offline")) {
                                                sender.sendMessage(setColors(getFileAccessor().getLang().getString("other.offline-no-perms")));
                                            ...
                                            banManager.preformBanByIp(args[1], BanType.TIMED_NOT_IP, getReason(args, 3), sender, time, false, "tempban", false);
                                            return;
                                        }
                                    } else {
                                        if (!sender.hasPermission("functionalservercontrol.temp-ban.offline")) {
                                        ...
                                        banManager.preformBanByIp(args[1], BanType.TIMED_NOT_IP, getReason(args, 3), sender, time, false, "tempban", true);
                                        return;
                                    }
                                });
                                return true;
                            ...
                            if (!OtherUtils.isNotNullPlayer(args[1])) {
                                ...
                                Bukkit.getScheduler().runTaskAsynchronously(this.plugin, () -> {
                                    banManager.preformBan(args[1], BanType.TIMED_NOT_IP, getReason(args, 3), sender, time, false, "tempban");
                                });
                                return true;
                            } else {
                                if (!player.isOnline()) {
                    ...
                    if (timeSettingsAccessor.getTimeChecker().checkInputTimeArgument(args[1])) {
                            ...
                            Bukkit.getScheduler().runTaskAsynchronously(this.plugin, () -> {
                                if(OtherUtils.isNotNullIp(args[0])) {
                                    if(OtherUtils.getPlayerByIP(args[0]) != null) {
                                        ...
                                        banManager.preformBan(player, BanType.TIMED_NOT_IP, getReason(args, 2), sender, time, true, "tempban");
                                        return;
                                    } else {
                                        if (!sender.hasPermission("functionalservercontrol.temp-ban.offline")) {
                                            sender.sendMessage(setColors(getFileAccessor().getLang().getString("other.offline-no-perms")));
                                        ...
                                        banManager.preformBanByIp(args[0], BanType.TIMED_NOT_IP, getReason(args, 2), sender, time, true, "tempban", false);
                                        return;
                                    }
                                } else {
                                    if (!sender.hasPermission("functionalservercontrol.temp-ban.offline")) {
                                    ...
                                    banManager.preformBanByIp(args[0], BanType.TIMED_NOT_IP, getReason(args, 2), sender, time, true, "tempban", true);
                                    return;
                                }
                            });
                            return true;
                        ...
                        if (!OtherUtils.isNotNullPlayer(args[0])) {
                            ...
                            Bukkit.getScheduler().runTaskAsynchronously(this.plugin, () -> {
                                banManager.preformBan(args[0], BanType.TIMED_NOT_IP, getReason(args, 2), sender, time, true, "tempban");
                            });
                            return true;
                        } else {
                            if (!player.isOnline()) {
