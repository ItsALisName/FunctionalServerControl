...
import static by.alis.functionalbans.spigot.Additional.Containers.StaticContainers.getCheckingCheatsPlayers;
...
public class CheatCheckerManager {
    ...
    public void startCheck(CommandSender initiator, Player player, @Nullable String reason, int checkTime) {
        ...
        if(reason.equalsIgnoreCase(getGlobalVariables().getDefaultReason())) {
            if(!getConfigSettings().isCheatsCheckAllowedWithoutReason() && !initiator.hasPermission("functionalbans.use.no-reason")) {
        ...
        if(player.hasPermission("functionalbans.cheat-check.bypass") && !initiator.hasPermission("functionalbans.bypass-break")) {
    ...
    public void stopCheck(CommandSender initiator, Player player) {
        if(isPlayerChecking(player)) {
            if(getCheckingCheatsPlayers().getInitiatorsAndHisPlayers().get(player) == initiator) {
                ...
                if(!initiator.hasPermission("functionalbans.cheat-check.other")) {
