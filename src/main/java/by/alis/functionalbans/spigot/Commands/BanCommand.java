            ...
            if(args.length >= 1) {
                            ...
                            for (Player target : Bukkit.getOnlinePlayers()) {
                                if (!target.hasPermission("functionalbans.ban.bypass")) {
                                    ...
                                    if(!sender.hasPermission("functionalbans.time-bypass")) {
                                        this.banManager.banPlayer(((OfflinePlayer) target), BanType.TIMED_NOT_IP, getReason(args, 2), sender, this.timeSettingsAccessor.getTimeManager().getMaxPlayerPunishTime((Player)sender), false);
                                    } else {
                                        this.banManager.banPlayer(((OfflinePlayer) target), BanType.PERMANENT_NOT_IP, getReason(args, 2), sender, -1, false);
                                    }
                                }
                            }
                        ...
                        if (getConfigSettings().isUnsafeActionsConfirmation()) {
                            ...
                            for (Player target : Bukkit.getOnlinePlayers()) {
                                if (!target.hasPermission("functionalbans.ban.bypass")) {
                                    ...
                                    this.cache.setUnsafeBannedPlayers(((OfflinePlayer) target), sender);
                                    if(!sender.hasPermission("functionalbans.time-bypass")) {
                                        this.banManager.banPlayer(((OfflinePlayer) target), BanType.TIMED_NOT_IP, getReason(args, 2), sender, this.timeSettingsAccessor.getTimeManager().getMaxPlayerPunishTime((Player)sender), true);
                                    } else {
                                        this.banManager.banPlayer(((OfflinePlayer) target), BanType.PERMANENT_NOT_IP, getReason(args, 2), sender, -1, true);
                                    }
                                }
                            }
                            sender.sendMessage(setColors(this.fileAccessor.getLang().getString("unsafe-actions.unsafe-ban-success").replace("%1$f", String.valueOf(a))));
                ...
                if (args.length == 1) {
                    ...
                    if(args[0].equalsIgnoreCase("-s")) {
                        ...
                        sender.sendMessage(setColors(this.fileAccessor.getLang().getString("other.flags.s.ban")));
                        return true;
                    }
                    ...
                    if(player == null) {
                        ...
                        if(!sender.hasPermission("functionalbans.time-bypass")) {
                            this.banManager.banPlayer(args[0], BanType.TIMED_NOT_IP, getGlobalVariables().getDefaultReason(), sender, this.timeSettingsAccessor.getTimeManager().getMaxPlayerPunishTime((Player)sender), true);
                        } else {
                            this.banManager.banPlayer(args[0], BanType.PERMANENT_NOT_IP, getGlobalVariables().getDefaultReason(), sender, -1, true);
                        }
                    } else {
                        if(player.isOnline()) {
                        ...
                        if(!sender.hasPermission("functionalbans.time-bypass")) {
                            this.banManager.banPlayer(player, BanType.TIMED_NOT_IP, getGlobalVariables().getDefaultReason(), sender, this.timeSettingsAccessor.getTimeManager().getMaxPlayerPunishTime((Player)sender), true);
                        } else {
                            this.banManager.banPlayer(player, BanType.PERMANENT_NOT_IP, getGlobalVariables().getDefaultReason(), sender, -1, true);
                        }
                    }
                    return true;
                }
                ...
                if(args.length >= 2) {
                    if(args[0].equalsIgnoreCase("-s") && args.length == 2) {
                        ...
                        if(player == null) {
                            ...
                            if(!sender.hasPermission("functionalbans.time-bypass")) {
                                this.banManager.banPlayer(args[1], BanType.TIMED_NOT_IP, getGlobalVariables().getDefaultReason(), sender, this.timeSettingsAccessor.getTimeManager().getMaxPlayerPunishTime((Player)sender), false);
                            } else {
                                this.banManager.banPlayer(args[1], BanType.PERMANENT_NOT_IP, getGlobalVariables().getDefaultReason(), sender, -1, false);
                            }
                        } else {
                            if(player.isOnline()) {
                                if(player.getPlayer().hasPermission("functionalbans.ban.bypass") && !sender.hasPermission("functionalbans.bypass-break")) {
                            ...
                            if(!sender.hasPermission("functionalbans.time-bypass")) {
                                this.banManager.banPlayer(player, BanType.TIMED_NOT_IP, getGlobalVariables().getDefaultReason(), sender, this.timeSettingsAccessor.getTimeManager().getMaxPlayerPunishTime((Player)sender), false);
                            } else {
                                this.banManager.banPlayer(player, BanType.PERMANENT_NOT_IP, getGlobalVariables().getDefaultReason(), sender, -1, false);
                            }
                        }
                        return true;
                    }
                    ...
                    if(args[0].equalsIgnoreCase("-s") && args.length > 2) {
                        ...
                        if(player == null) {
                            ...
                            if(!sender.hasPermission("functionalbans.time-bypass")) {
                                this.banManager.banPlayer(args[1], BanType.TIMED_NOT_IP, getReason(args, 2), sender, this.timeSettingsAccessor.getTimeManager().getMaxPlayerPunishTime((Player)sender), false);
                            } else {
                                this.banManager.banPlayer(args[1], BanType.PERMANENT_NOT_IP, getReason(args, 2), sender, -1, false);
                            }
                        } else {
                            if(player.isOnline()) {
                                if(player.getPlayer().hasPermission("functionalbans.ban.bypass") && !sender.hasPermission("functionalbans.bypass-break")) {
                            ...
                            if(!sender.hasPermission("functionalbans.time-bypass")) {
                                this.banManager.banPlayer(player, BanType.TIMED_NOT_IP, getReason(args, 2), sender, this.timeSettingsAccessor.getTimeManager().getMaxPlayerPunishTime((Player)sender), false);
                            } else {
                                this.banManager.banPlayer(player, BanType.PERMANENT_NOT_IP, getReason(args, 2), sender, -1, false);
                            }
                        }
                        return true;
                    }
                    ...
                    if(this.timeSettingsAccessor.getTimeChecker().checkInputTimeArgument(args[1]) && args.length == 2) {
                        ...
                        if(!sender.hasPermission("functionalbans.time-bypass")) {
                            if(this.timeSettingsAccessor.getTimeManager().convertToMillis(args[1]) > this.timeSettingsAccessor.getTimeManager().getMaxPlayerPunishTime((Player) sender)) {
                    ...
                    if(player == null) {
                        ...
                        if(!sender.hasPermission("functionalbans.time-bypass")) {
                            this.banManager.banPlayer(args[1], BanType.TIMED_NOT_IP, getReason(args, 1), sender, this.timeSettingsAccessor.getTimeManager().getMaxPlayerPunishTime((Player)sender), true);
                        } else {
                            this.banManager.banPlayer(args[1], BanType.PERMANENT_NOT_IP, getReason(args, 1), sender, -1, true);
                        }
                        return true;
                    } else {
                        if(player.isOnline()) {
                        ...
                        if(!sender.hasPermission("functionalbans.time-bypass")) {
                            this.banManager.banPlayer(player, BanType.TIMED_NOT_IP, getReason(args, 1), sender, this.timeSettingsAccessor.getTimeManager().getMaxPlayerPunishTime((Player)sender), true);
                        } else {
                            this.banManager.banPlayer(player, BanType.PERMANENT_NOT_IP, getReason(args, 1), sender, -1, true);
                        }
                        return true;
                    }
